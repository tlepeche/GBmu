...
void Cpu_z80::_setOpcodeMap()
{
	_opcodeMap = {
		...
		(t_opcode){0x18, 0x00, 8, 8, 2, std::bind(&Cpu_z80::JR_n, this),     	"JR n",			0x0000},
		...
		(t_opcode){0x20, 0xc0, 8, 8 , 2, std::bind(&Cpu_z80::JR_NZ_n, this),   "JR NZ, n",		0x0000},
		...
		(t_opcode){0x23, 0x00, 12 , 12 , 1, std::bind(&Cpu_z80::INC_HL, this),    "INC (HL)",		0x0000},
		...
		(t_opcode){0x28, 0x80, 8, 8 , 2, std::bind(&Cpu_z80::JR_Z_n, this),    "JR Z, n",		0x0000},
		...
		(t_opcode){0x2c, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::INC_L, this),    	"INC L",		0x0000},
		(t_opcode){0x2d, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::DEC_L, this),   	"DEC L",		0x0000},
		(t_opcode){0x2e, 0x00, 16 , 16 , 2, std::bind(&Cpu_z80::LD_L_n, this),    "LD L, n",		0x0000},
		(t_opcode){0x2f, 0x00, 4 , 4 , 1, std::bind(&Cpu_z80::CPL, this),    	"CPL",			0x0000},
		(t_opcode){0x30, 0x70, 8, 8 , 2, std::bind(&Cpu_z80::JR_NC_n, this),   "JR NZ, n",		0x0000},
		...
		(t_opcode){0x34, 0x00, 8, 8, 1, std::bind(&Cpu_z80::INC_HLF, this),   "INC (HL)",		0x0000},
		(t_opcode){0x35, 0x00, 8, 8, 1, std::bind(&Cpu_z80::DEC_HLF, this),   "DEC (HL)",		0x0000},
		(t_opcode){0x36, 0x00, 16, 16, 2, std::bind(&Cpu_z80::LD_HL_n, this),   "LD (HL), n",	0x0000},
		(t_opcode){0x37, 0x00, 4 , 4 , 1, std::bind(&Cpu_z80::SCF, this),    	"SCF",			0x0000},
		(t_opcode){0x38, 0x10, 8, 8 , 2, std::bind(&Cpu_z80::JR_C_n, this),    "JR C, n",		0x0000},
		...
		(t_opcode){0x3c, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::INC_A, this),    	"INC A",		0x0000},
		(t_opcode){0x3d, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::DEC_A, this),    	"DEC A",		0x0000},
		(t_opcode){0x3e, 0x00, 16 , 16 , 2, std::bind(&Cpu_z80::LD_A_n, this),    "LD A, n",		0x0000},
		(t_opcode){0x3f, 0x00, 4 , 4 , 1, std::bind(&Cpu_z80::CCF, this),    	"CCF",			0x0000},
		(t_opcode){0x40, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::LD_B_B, this),    "LD B, B",		0x0000},
		(t_opcode){0x41, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::LD_B_C, this),    "LD B, C",		0x0000},
		(t_opcode){0x42, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::LD_B_D, this),    "LD B, D",		0x0000},
		(t_opcode){0x43, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::LD_B_E, this),    "LD B, E",		0x0000},
		(t_opcode){0x44, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::LD_B_H, this),    "LD B, H",		0x0000},
		(t_opcode){0x45, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::LD_B_L, this),    "LD B, L",		0x0000},
		(t_opcode){0x46, 0x00, 16 , 16 , 1, std::bind(&Cpu_z80::LD_B_HL, this),   "LD B, HL",		0x0000},
		...
		(t_opcode){0xbc, 0x00, 8 , 8 , 1, std::bind(&Cpu_z80::CP_H, this),    	"CP H",			0x0000},
		...
		(t_opcode){0xc0, 0xc0,  8, 8 , 1, std::bind(&Cpu_z80::RET_NZ, this),    "RET NZ",		0x0000},
		(t_opcode){0xc1, 0x00, 12, 12, 1, std::bind(&Cpu_z80::POP_BC, this),    "POP BC",		0x0000},
		(t_opcode){0xc2, 0xc0, 12, 12, 3, std::bind(&Cpu_z80::JP_NZ_n, this),   "JP NZ, nn",	0x0000},
		(t_opcode){0xc3, 0x00, 12, 12, 3, std::bind(&Cpu_z80::JP_n, this),     	"JP nn",		0x0000},
		(t_opcode){0xc4, 0xc0, 12, 12, 3, std::bind(&Cpu_z80::CALL_NZ_n, this), "CALL NZ, nn",	0x0000},
		...
		(t_opcode){0xc7, 0x00, 32, 32, 1, std::bind(&Cpu_z80::RST_00H, this),   "RST_00H",		0x0000},
		(t_opcode){0xc8, 0x80,  8, 8 , 1, std::bind(&Cpu_z80::RET_Z, this),    	"RET Z",		0x0000},
		(t_opcode){0xc9, 0x00,  8,  8, 1, std::bind(&Cpu_z80::RET, this),    	"RET",			0x0000},
		(t_opcode){0xca, 0x80, 12, 12, 3, std::bind(&Cpu_z80::JP_Z_n, this),    "JP Z, nn",		0x0000},
		(t_opcode){0xcb, 0x00, 4 , 4 , 1, std::bind(&Cpu_z80::PREFIX_CB, this), "PREFIX CB",	0x0000},
		(t_opcode){0xcc, 0x80, 12, 12, 3, std::bind(&Cpu_z80::CALL_Z_n, this),  "CALL Z, nn",	0x0000},
		(t_opcode){0xcd, 0x00, 12, 12, 3, std::bind(&Cpu_z80::CALL_n, this),    "CALL nn",		0x0000},
		(t_opcode){0xce, 0x00, 8 , 8 , 2, std::bind(&Cpu_z80::ADC_A_n, this),   "ADC A, n",		0x0000},
		(t_opcode){0xcf, 0x00, 32, 32, 1, std::bind(&Cpu_z80::RST_08H, this),   "RST 08H",		0x0000},
		(t_opcode){0xd0, 0x50,  8, 8 , 1, std::bind(&Cpu_z80::RET_NC, this),    "RET NC",		0x0000},
		(t_opcode){0xd1, 0x00, 12, 12, 1, std::bind(&Cpu_z80::POP_DE, this),    "POP DE",		0x0000},
		(t_opcode){0xd2, 0x50, 12, 12, 3, std::bind(&Cpu_z80::JP_NC_n, this),   "JP NC, nn",	0x0000},
		(t_opcode){0xd3, 0x00, 0 , 0 , 0, NULL, "",	0x0000},
		(t_opcode){0xd4, 0x50, 12, 12, 3, std::bind(&Cpu_z80::CALL_NC_n, this), "CALL NC, nn",	0x0000},
		...
		(t_opcode){0xd7, 0x00, 32, 32, 1, std::bind(&Cpu_z80::RST_10H, this),   "RST 10H",		0x0000},
		(t_opcode){0xd8, 0x10,  8,  8, 1, std::bind(&Cpu_z80::RET_C, this),    	"RET C",		0x0000},
		(t_opcode){0xd9, 0x00,  8,  8, 1, std::bind(&Cpu_z80::RETI, this),    	"RETI",			0x0000},
		(t_opcode){0xda, 0x10, 12, 12, 3, std::bind(&Cpu_z80::JP_C_n, this),    "JP C, nn",		0x0000},
		(t_opcode){0xdb, 0x00, 0 , 0 , 0, NULL, "",	0x0000},
		(t_opcode){0xdc, 0x10, 12, 12, 3, std::bind(&Cpu_z80::CALL_C_n, this),  "CALL C, nn",	0x0000},
		(t_opcode){0xdd, 0x00, 0 , 0 , 0, NULL, "",	0x0000},
		(t_opcode){0xde, 0x00, 8 , 8 , 2, std::bind(&Cpu_z80::SBC_A_n, this),   "SBC A, nn",	0x0000},
		(t_opcode){0xdf, 0x00, 32, 32, 1, std::bind(&Cpu_z80::RST_18H, this),   "RST 18H",		0x0000},
		...
		(t_opcode){0xe7, 0x00, 32, 32, 1, std::bind(&Cpu_z80::RST_20H, this),   "RST 20H",		0x0000},
		...
		(t_opcode){0xef, 0x00, 32, 32, 1, std::bind(&Cpu_z80::RST_28H, this),   "RST 28H",		0x0000},
		...
		(t_opcode){0xf7, 0x00, 32, 32, 1, std::bind(&Cpu_z80::RST_30H, this),   "RST 30H",		0x0000},
		...
		(t_opcode){0xff, 0x00, 32, 32, 1, std::bind(&Cpu_z80::RST_38H, this),   "RST 38H",		0x0000}
	};
}
